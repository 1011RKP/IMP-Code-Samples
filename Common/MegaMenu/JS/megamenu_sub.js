...
function sortByShowOrderProperty(property) {
    return function (a, b) {
        var textA = a.Show_x0020_Order != null && a.Show_x0020_Order != "" ? parseInt(a.Show_x0020_Order) : "";
        var textB = b.Show_x0020_Order != null && b.Show_x0020_Order != "" ? parseInt(b.Show_x0020_Order) : "";
        return (textA  < textB ) ? -1 : (textA  > textB  ) ? 1 : 0;
    };
}
...
var intranetMenuSubAppCtrl= function (SPDataService, $filter, $timeout) {
    ...
    model.TopMenuItems = [];
    model.MenuItems = [];
    model.ListLink=_spPageContextInfo.webAbsoluteUrl+"/Lists/SideNavigation/AllItems.aspx" ;
    ...
    model.buildNavTree = function (parentID, menuType) {
        ...
        if (menuType == 'menuList') {
            ...
            firstLevel = items;
            firstLevel  =  firstLevel.sort(sortByShowOrderProperty('Show_x0020_Order'))
        }
        for (var i = 0; i < firstLevel.length ; i++) {
...
intranetMenusubApp.component('megamenusubTempalte', {
    templateUrl: '/sites/common/SiteAssets/MegaMenu/HTML/widgetTemplate/megaMenuTemplate_sub.html',
    controllerAs: "model",
...
intranetMenusubApp.factory('SPDataService', ['$http', '$q', function ($http, $q) {
    ...
    service.getTopMenuItems = function () {
       ...
       var restUrl =_spPageContextInfo.siteAbsoluteUrl + "/_api/web/lists/getbytitle('SideNavigation')/items" + select + expand + filter;
    ...
    service.IsSiteAdmin = function () {
        var restUrl =_spPageContextInfo.webAbsoluteUrl + "/_api/web/currentUser/?$select=IsSiteAdmin";
        var dfd = $q.defer();
        $.ajax({
            url: restUrl,
            type: "GET",
